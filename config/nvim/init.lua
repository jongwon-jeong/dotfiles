...
do
  ...
  if ok_blink then
    ...
    local blink_disabled_filetypes = {
      -- Keep explicit filetype switches here for quick per-language completion toggles.
      python = false,
    }
...
do
  ...
  if ok_devicons then devicons.setup {
    color_icons = true,
  } end
end
-- }}}
...
vim.keymap.set(
  ...
  '<leader>cp',
  ':cprevious<cr>zz',
  { desc = 'Go to previous quickfix item and center' }
)
