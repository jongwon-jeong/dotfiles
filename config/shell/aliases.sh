...
if [[ "$(uname)" = "Linux" ]]; then
  alias cp1='cp --force --archive --verbose'
  alias cpa='cp --archive --verbose'
  alias cpr='cp --recursive --verbose'
  alias cpclean='cp --force --no-preserve=all --recursive --verbose'
...
alias v='vim'
alias vd='vimdiff'
alias vdc='cd ~/proj/personal/dotfiles/ && vimdiff ~/.dotfiles/config/nvim/init.lua config/nvim/init.lua'
alias vdz='cd ~/proj/personal/dotfiles/ && vimdiff ~/.zshrc home/.zshrc'
alias vda='cd ~/proj/personal/dotfiles/ && vimdiff ~/.dotfiles/config/shell/aliases.sh config/shell/aliases.sh'
...
upgrade_all_managers() {
  ...
  command -v cargo >/dev/null 2>&1 && cargo_update_all
...
common_excludes=(.git node_modules dist build .next .cache .turbo .vite coverage target __pycache__ .venv)

if command -v eza >/dev/null; then
  readonly ezaExclude="$(_join_by '|' "${common_excludes[@]}")"
...
if command -v tree >/dev/null; then
  readonly treeExclude="$(_join_by '|' "${common_excludes[@]}")"
...
if [ -n "${_EZA_CMD}" ]; then
  ...
  alias lt="${_EZA_CMD} -T --all --icons --ignore-glob=\"${ezaExclude}\""
  alias ll="ls -hal --git"
else
...
histrm() {
  ...
  del ~/.bash_history 2>/dev/null
  del ~/.zsh_history 2>/dev/null
