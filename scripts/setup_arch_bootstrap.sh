...
install_base_packages() { # {{{
  ...
  # Keep the stock GNOME group as the install source, then remove personal
  # desktop apps that are intentionally replaced by explicit packages below.
  remove_package_if_installed epiphany
  install_package \
    gdm \
    gnome-shell \
    gnome-session \
  ...
  replace_package_before_install pipewire-jack jack2 || true
...
install_codex_cli() { # {{{
  ...
  # Codex release resolution uses GitHub APIs and can hit unauthenticated rate
  # limits during a full bootstrap. Keep it best-effort so the core toolchain
  # does not fail just because this optional CLI needs a later retry.
  # shellcheck disable=SC2016
