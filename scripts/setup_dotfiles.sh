...
validate_platform() {
  ...
  echo "   setup_dotfiles.sh supports Linux and macOS only."
  return 1
}
...
create_symlink() {
  ...
  if is_linux; then
    ...
    ln -fhnsv "${source_path}" "${target_path}"
  else
    echo "ERROR: Unsupported platform while linking: $(uname)"
    return 1
  fi
}
...
copy_files() {
  ...
  if is_linux; then
    ...
    echo "ERROR: Unsupported platform while copying: $(uname)"
    return 1
  fi
}
  ...
  if [ "${dotfiles_root}" != "${dotfiles_base}" ]; then
    ...
    if command -v rsync &>/dev/null; then
      ...
      for item in "${dotfiles_root}"/*; do
        ...
        copy_files "${item}" "${dotfiles_base}" || failed=true
      done
      shopt -u dotglob
      if [[ "${failed}" == "true" ]]; then
        return 1
...
symlink_dotfiles() {
  ...
  if [ -d "${repo_config_dir}" ]; then
    ...
    for item_path in "${repo_config_dir}"/*; do
      ...
      if [ -d "${item_path}" ]; then
        link_recursive "${item_path}" "${HOME}/.config/${item_name}" || failed=true
      else
        create_symlink "${item_path}" "${HOME}/.config/${item_name}" || failed=true
      fi
    done
    shopt -u dotglob nullglob
  fi
