...
ubuntu_install_basic_packages() { # {{{
  if [ "$PM" = "apt" ]; then
    install_packages \
      ...
      ripgrep fd-find tree htop trash-cli entr jq ffmpeg \
      qemu-kvm libvirt-daemon-system virt-manager ovmf swtpm swtpm-tools virt-viewer
...
ubuntu_install_google_chrome() { # {{{
  if [ "$PM" = "apt" ]; then
    ...
    DOWNLOAD_PATH="${DOWNLOAD_DIR}/${CHROME_DEB}"

    mkdir -p "${DOWNLOAD_DIR}"
    wget -P "${DOWNLOAD_DIR}" "https://dl.google.com/linux/direct/${CHROME_DEB}"
...
main() { # {{{
  ...
  if [ "${is_wsl:-0}" -ne 1 ]; then
    tasks+=(ubuntu_install_google_chrome)
